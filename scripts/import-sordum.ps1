...
#$baseUrl = 'https://www.sordum.org/category/some-small-tools/';
$baseUrl = 'https://www.sordum.org/downloads/'
$content = $client.DownloadString($baseUrl) -replace '<!--(.+?)-->',''
$matches = [regex]::Matches($content, 'href="([^"]+?)">(.+?)</a><span title="Number of downloads">')
$targetDir = $PSScriptRoot + '\..\packages\sordum'
